...
void output(const matched_line* m_lines, const cmd_args_data* cad,
  ...
  if (cad->flags.l) {
    ...
    if (cad->flags.c)
      ...
      print_matched_lines_with_additional_info_if_needed(m_lines, cad,
                                                         search_file);
  }
}
...
void print_matched_lines_with_additional_info_if_needed(
  ...
  size_t n_files = get_count_of_files_from_cmd_args(cad);
  size_t l_idx =
      find_idx_of_last_matched_line_with_file_name(search_file, m_lines);
  if (l_idx != __SIZE_MAX__) {
    for (size_t i = find_idx_of_first_matched_line_with_file_name(search_file,
                                                                  m_lines);
         i <= l_idx; ++i) {
