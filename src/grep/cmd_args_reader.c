...
bool set_cmd_arg_flags(cmd_args_data* cad, int argc, char** argv) {
  ...
  int flag = 0;
  bool status = true;
  // ':' as the first char in optstring allow getopt to return ':' instead of
  // '?' to indicate a missing option argument.
  while (status && (flag = getopt_long(argc, argv, ":e:ivclnhsf:o",
                                       long_options, NULL)) != -1) {
    if (flag == 'e') {
      cad->flags.e = 1;
      if (!add_pattern_to_patterns_string(&cad->patterns, optarg))
        status = false;
    } else if (flag == 'i') {
      cad->flags.i = 1;
    } else if (flag == 'v') {
      ...
      cad->flags.f = 1;
      if (!add_str_to_str_arr_dynamically(&cad->pattern_files, optarg))
        status = false;
    } else if (flag == 'o') {
      cad->flags.o = 1;
    } else if (flag == ':') {
...
bool set_cmd_arg_files(cmd_args_data* cad, int argc, char** argv) {
  ...
  if (optind == argc) {
    ...
    if (!cad->flags.e && !cad->flags.f)
      if (!add_pattern_to_patterns_string(&cad->patterns, argv[optind++]))
        status = false;
    ...
    for (int i = optind; status && i < argc; ++i)
      if (!add_str_to_str_arr_dynamically(&cad->search_files, argv[i]))
        status = false;
  }
  return status;
}