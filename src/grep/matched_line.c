...
bool fill_matched_line(matched_line* ml_ptr, size_t line_number,
...
bool append_matched_line(matched_line** m_lines,
                         const matched_line* src_m_line) {
...
size_t find_idx_of_last_matched_line_with_file_name(
    const char* file_name, const matched_line* m_lines) {
  if (!file_name) return __SIZE_MAX__;

  size_t first_idx =
      find_idx_of_first_matched_line_with_file_name(file_name, m_lines);
  size_t i = first_idx;
  if (first_idx != __SIZE_MAX__)
    while (!is_last(&m_lines[i]) &&
           !strcmp(m_lines[first_idx].file_name, m_lines[i].file_name))
      ++i;

  return first_idx == __SIZE_MAX__ ? __SIZE_MAX__ : i - 1;
}

size_t find_idx_of_first_matched_line_with_file_name(
    const char* file_name, const matched_line* m_lines) {
  if (!file_name) return __SIZE_MAX__;

  size_t idx = 0;
  while (!is_last(&m_lines[idx]) && strcmp(file_name, m_lines[idx].file_name))
    ++idx;

  return is_last(&m_lines[idx]) ? __SIZE_MAX__ : idx;
}

size_t get_matched_lines_count_on_file(const char* file_name,
                                       const matched_line* m_lines) {
  if (!file_name || !m_lines) return __SIZE_MAX__;

  size_t first_idx =
      find_idx_of_first_matched_line_with_file_name(file_name, m_lines);
  size_t last_idx =
      find_idx_of_last_matched_line_with_file_name(file_name, m_lines);
  size_t n = 1;
  if (first_idx != __SIZE_MAX__) {
