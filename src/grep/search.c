...
bool set_matched_lines_with_patterns_from_file(matched_line** m_lines_ptr,
  ...
  regex_t regex;
  size_t error = 0;
  if ((error = regcomp(&regex, cad->patterns,
                       REG_EXTENDED | (cad->flags.i ? REG_ICASE : 0))) >
      REG_NOMATCH) {
...
void output_regex_error(size_t error, regex_t* regex_ptr) {
  // If errbuf_size is 0 returns the size of the buffer needed to hold the
  // generated string.
...
bool regexec_on_line(regex_t* regex_ptr, const char* line,
                     const cmd_args_data* cad, matched_line** m_lines_ptr,
                     const char* search_file, size_t line_num) {
  bool status = true;
  regmatch_t rm[1];
  int error = regexec(regex_ptr, line, 1, rm, 0);
  if ((error == 0 && !cad->flags.v) || (error == REG_NOMATCH && cad->flags.v)) {
    if (cad->flags.o) {
      status = set_all_matches_from_line(m_lines_ptr, regex_ptr, line, rm,
                                         search_file, line_num);
    } else {
...
bool set_all_matches_from_line(matched_line** m_lines, regex_t* regex,
                               const char* str, regmatch_t* rm,
