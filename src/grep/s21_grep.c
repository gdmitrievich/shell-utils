...
void s21_grep(int argc, char** argv) {
  ...
  if (cad.flags.f)
    status = set_regexes_retrieved_from_files(&cad.patterns, cad.pattern_files);
  if (status) status = process_files_one_by_one_as_grep_utility(&cad);
  free_cad(cad);

  if (!status && errno != 0 && !(cad.flags.s && errno == ENOENT))
    print_error("grep", NULL);
}
...
bool process_files_one_by_one_as_grep_utility(const cmd_args_data* cad) {
  ...
  for (size_t i = 0; status && cad->search_files[i] != NULL; ++i) {
    ...
    status = set_matched_lines_with_patterns_from_file(
        &m_lines, cad, cad->search_files[i], &file_found);
